...
export function ProjectsProvider({ children }: ProjectsProviderProps) {
  ...
  function createFile(filePath: string, response: any) {
  ...
  async function installProjectFiles(filePath: string, projectInput: Project) {
    ...
    for (const pack of packages) {
      ...
      await api.get(`package/${pack.name}/file`)
        .then(async (response) => {
          const buff = Buffer.from(response.data, "base64");
          await fsPromises.writeFile(path.join(filePath, `${pack.name}.zip`), buff, (err) => {
            if (err) { console.log(err); throw err };
          })
          // await createFile(path.join(filePath, `${pack.name}.zip`), response);
        })
    }
    ...
    for (const pack of packages) {
      ...
      if (os.platform() === 'darwin') {
        ...
        await extract(path.join(filePath, `${pack.name}.zip`), { dir: filePath })
      }

      console.log(`Deleting file ${pack.name}...`);
      await fsPromises.unlink(path.join(filePath, `${pack.name}.zip`));
    ...
    await dialog.showOpenDialog({ properties: ['openDirectory'] })
      .then(async response => {
        try {
          ...
          const filePath = path.join(projectPath, `eBankit.${projectInput.alias}`, 'Main', 'Source')
